...
const PatientView: React.FC = () => {
  ...
  const [requestStatus, setRequestStatus] = useState<'idle' | 'calling' | 'help_requested' | 'support_requested'>('idle');

  return (
    <div className="max-w-5xl mx-auto p-6 md:p-12 space-y-12 senior-mode">
            ...
            <div key={appt.id} className="medical-card border-2 border-slate-200 overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-2 gap-8 border-y border-slate-100 py-6 mb-8">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-800">
                      {new Date(appt.datetime).toLocaleDateString('en-US', { weekday: 'short', month: 'long', day: 'numeric' })}
                    </p>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  >
                    {nextRide ? 'Logistics Confirmed' : 'Request Transport'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {nextRide && (
            <section className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-lg font-bold text-slate-800 uppercase tracking-wider">Transit Details</h2>
              </div>
              <div className="bg-slate-900 text-white rounded-lg p-8 shadow-xl border-b-8 border-blue-600">
                <div className="flex justify-between items-start mb-6">
                   <div>
                     <p className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.2em]">Vehicle Status</p>
                     <h3 className="text-2xl font-bold mt-1 leading-tight">{nextRide.driverName || 'Finding Coordinator...'}</h3>
                   </div>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex gap-4 items-center opacity-80">
                    ...
                    <p className="text-sm font-medium">Pickup from: {nextRide.pickupLocation}</p>
                  </div>
                  <div className="flex gap-4 items-center opacity-80">
                    <div className="w-2 h-2 rounded-full bg-blue-400" />
                    <p className="text-sm font-medium">Destination: {nextRide.destination}</p>
                  </div>
                </div>
                <button 
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <button 
                ...
                className={`p-10 rounded border-2 text-center transition-all ${requestStatus === 'help_requested' ? 'bg-rose-700 border-rose-700 text-white' : 'bg-white border-slate-200 text-rose-600 hover:border-rose-300'}`}
              >
                <p className="text-3xl font-bold mb-2 uppercase">Emergency</p>
                <p className="text-xs font-bold uppercase tracking-widest opacity-60">Immediate Clinical Alert</p>
              </button>
              <button 
                onClick={() => { store.requestPrivateSupport(patient.id); setRequestStatus('support_requested'); }}
                className={`p-10 rounded border-2 text-center transition-all ${requestStatus === 'support_requested' ? 'bg-blue-800 border-blue-800 text-white' : 'bg-white border-slate-200 text-blue-800 hover:border-blue-300'}`}
              >
                <p className="text-3xl font-bold mb-2 uppercase">Request Talk</p>
                <p className="text-xs font-bold uppercase tracking-widest opacity-60">Connect with Care Team</p>
              </button>
            </div>
          </section>
        </div>
